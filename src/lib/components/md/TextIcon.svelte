<script>
  ...
  let { text = "word", icon = undefined, fill = "none", stroke = "currentColor", bgColor = "unset", color = "F3F4F6" } = $props();
  ...
  let d = $derived(icon ? icons[icon] : undefined);
  let isDark = $state(false);

  if (browser) {
    import("./isDark.js").then((module) => {
      module.isDark.subscribe((value) => {
        isDark = value;
      });
    });
  }

  let finalColor = $derived(!isDark && color === "white" ? "black" : color);
</script>
